...
const partition = async (data, file) => await client.general.partition({
    partitionParameters: {
      files: {
        content: data,
        fileName: file,
...
async function main() {
    logger.profile(`Overall Runtime with strategy ${strategy} and split pages ${splitPages}`)
    ...
    for (const file of files) {
      try {
        ...
        const partitionedFile = await partition(fileBuffer, file)
        const writePartitionResults = await writeResults(outputPath, file, partitionedFile)
